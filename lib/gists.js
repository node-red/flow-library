...
async function addGist(data) {
    ...
    if (originalFiles['flow.json'].truncated) {
        throw new Error("Flow file too big - flow library does not currently support truncated files from github api");
    }
    if (!originalFiles['README.md']) {
        throw new Error("Missing file README.md");
    }
    if (originalFiles['README.md'].truncated) {
        throw new Error("README file too big - flow library does not currently support truncated files from github api");
    }
    ...
    db.flows.save(data,function(err,other) {
        ...
        const oldPath = path.join(GIST_DIR,data.id);
