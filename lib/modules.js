...
async function getLatestVersion(name) {
    if (blockList.indexOf(name) !== -1) {
        return Promise.reject(name+" : on the block list");
    }
    util.log(name,'getModuleInfo')
    let info = await getModuleInfo(name);
    util.log(name,'getModuleInfo DONE')
    return new Promise((resolve,reject) => {
        try {
            var nodePath = path.join(NODE_DIR,name);
    ...
    list.forEach(function(item) {
        ...
        promises.push(
            npmNodes.get(name).catch(function(err) {
                ...
                util.log(item.name,"getLatestVersion");
        ...
        return processUpdated(list).then(function(results) {
            ...
            results.forEach(function(d,i) {
                if (d.state === 'fulfilled' && d.value === undefined) {
                    promises.push(npmNodes.update(list[i].name,{refresh_requested:false}).catch(err => {}));
                }
            });
