            ...
            npmNodes.get({_id:1}).then(function(knownNodes) {
               knownNodes.forEach(function(r) {
                    if (!foundNodes[r._id]) {
                        ...
                        promises.push(removeFromDb(r._id));
                    }
                });
            });
            ...
            function processSlice() {
                ...
                if (fullList.length == 0) {
                    ...
                    promises = promises.concat(list.map(function(entry) {
                        return refreshModule(entry.name).then(saveToDb).otherwise(function(err) {
                            return removeFromDb(entry.name).then(function() {
                                throw err;
                            });
                        });
                    }));
