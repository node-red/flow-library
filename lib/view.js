    ...
    getForQuery: function(query) {
        ...
        if (query.collection) {
            ...
            if (query.username || query.npm_username) {
                ...
                if (query.npm_username && query.username !== query.npm_username) {

                } else {
                    findQuery.owners = {$elemMatch: {"name": query.username}}
                    countQuery.owners = countQuery.owners;
                }

                // findQuery["$or"] = [];
                // if (query.username) {
                //     findQuery["$or"].push({"owners.login":query.username})
                // }
                // findQuery["$or"].push({"owners": {$elemMatch: {"name": query.npm_username||query.username}}})
